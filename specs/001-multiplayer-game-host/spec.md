...
- **방장 퇴장**: 게임 방 전체의 방장(방 생성자)이 퇴장하면 다음 플레이어에게 자동으로 권한이 이전된다. 게임 진행 중 각 방(빨간 방/파란 방)의 리더가 퇴장하면 해당 방의 다음 플레이어에게 리더 권한이 이전된다.
...
- **FR-012**: 시스템은 방장이 설정한 인질 교환 패턴에 따라 라운드별 교환 가능한 인질 수를 자동으로 결정해야 한다. 방장은 게임 시작 전 다음 중 하나를 선택할 수 있다: (1) 고정 - 모든 라운드 1명, (2) 점진적 증가 - 라운드마다 1명씩 증가 (최대 3명), (3) 후반 집중 - 초반 1명, 후반 2-3명.
...
- 인질 교환 패턴의 기본값은 "점진적 증가"이며, 원본 게임 규칙을 따라 라운드 1-2는 1명, 라운드 3-4는 2명, 라운드 5+는 3명을 교환한다(방장이 다른 패턴으로 변경 가능).
