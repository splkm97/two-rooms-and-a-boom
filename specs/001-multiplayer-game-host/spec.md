...
- Q: 인질 교환의 "방장" 정의는? → A: 각 방마다 방장 존재 - 빨간 방 방장과 파란 방 방장이 각자 인질 선택
...
각 라운드마다 플레이어들은 빨간 방과 파란 방으로 자동 분배되며, 각 방에는 방장이 지정된다. 라운드 종료 시 각 방의 방장이 선택한 인질들이 방을 교환한다.
...
1. **Given** 게임이 시작되었을 때, **When** 라운드 1이 시작되면, **Then** 플레이어들이 빨간 방과 파란 방으로 가능한 균등하게 분배되고, 각 방에 방장이 자동으로 지정된다.
2. **Given** 플레이어가 빨간 방에 배정되었을 때, **When** 자신의 화면을 확인하면, **Then** 자신이 속한 방 이름, 같은 방에 있는 다른 플레이어 목록, 그리고 현재 방의 방장이 누구인지 표시된다.
3. **Given** 라운드가 종료되었을 때, **When** 빨간 방 방장과 파란 방 방장이 각자의 방에서 인질 교환을 위해 1명씩 선택하면, **Then** 선택된 플레이어들은 다음 라운드에서 반대편 방으로 이동한다.
...
- **방장 퇴장**: 게임 방 전체의 방장(방 생성자)이 퇴장하면 다음 플레이어에게 자동으로 권한이 이전된다. 게임 진행 중 각 방(빨간 방/파란 방)의 방장이 퇴장하면 해당 방의 다음 플레이어에게 방장 권한이 이전된다.
...
- **FR-024**: 시스템은 각 방(빨간 방, 파란 방)에 방장을 자동으로 지정해야 한다.
- **FR-025**: 시스템은 각 방의 방장이 누구인지 해당 방의 모든 플레이어에게 표시해야 한다.
- **FR-009**: 시스템은 각 라운드마다 제한 시간을 표시하고 카운트다운해야 한다.
- **FR-010**: 시스템은 라운드 타이머가 0이 되면 자동으로 다음 라운드로 전환해야 한다.
- **FR-011**: 시스템은 라운드 종료 시 각 방의 방장이 자신의 방에서 인질을 선택할 수 있는 인터페이스를 제공해야 한다.
...
- **방 (Room in Game)**: 방 이름(빨간 방/파란 방), 현재 라운드의 플레이어 목록, 방장 정보
- **인질 교환 (Hostage Exchange)**: 라운드 번호, 교환할 플레이어 목록, 빨간 방 방장 선택 정보, 파란 방 방장 선택 정보
