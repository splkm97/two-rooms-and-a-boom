...
1. **Given** 사용자가 홈 화면에 있을 때, **When** "방 만들기" 버튼을 클릭하면, **Then** 고유한 6자리 방 코드가 생성되고 방장으로 대기실에 입장한다.
2. **Given** 방 코드를 받은 플레이어가 홈 화면에 있을 때, **When** "방 참가" 버튼을 클릭하고 방 코드를 입력하면, **Then** 해당 방의 대기실로 입장하고 모든 참가자에게 실시간으로 표시된다.
3. **Given** 대기실에 3명의 플레이어가 있을 때, **When** 한 명이 나가면, **Then** 남은 플레이어들의 화면에서 해당 플레이어가 즉시 제거된다.
4. **Given** 대기실에 최소 6명의 플레이어가 있을 때, **When** 방장이 "게임 시작" 버튼을 클릭하면, **Then** 모든 플레이어가 게임 화면으로 전환된다.
5. **Given** 존재하지 않는 방 코드를 입력했을 때, **When** 입장을 시도하면, **Then** "방을 찾을 수 없습니다" 오류 메시지가 표시된다.
...
**Independent Test**: 게임 시작 후 각 플레이어가 자신의 화면에서 역할 카드를 확인하고, 레드 팀과 블루 팀이 적절히 분배되었는지, 대통령과 폭탄범이 정확히 한 명씩 존재하는지 검증한다.

**Acceptance Scenarios**:

1. **Given** 6명의 플레이어가 대기실에 있을 때, **When** 방장이 게임을 시작하면, **Then** 각 플레이어는 레드 팀 또는 블루 팀으로 배정되고 자신의 역할 카드를 확인한다.
2. **Given** 게임이 시작되었을 때, **When** 플레이어가 자신의 역할 카드를 탭하면, **Then** 역할 이름(예: 대통령, 폭탄범, 일반 레드, 일반 블루)과 팀 색상이 명확히 표시된다.
3. **Given** 10명의 플레이어로 게임이 시작되었을 때, **When** 역할 배분이 완료되면, **Then** 정확히 1명의 대통령(블루 팀)과 1명의 폭탄범(레드 팀)이 존재하며, 나머지는 일반 역할로 팀이 균형있게 배분된다.
...
각 라운드마다 플레이어들은 빨간 방과 파란 방으로 자동 분배되며, 라운드 종료 시 방장이 선택한 인질들이 방을 교환한다.

**Why this priority**: 게임의 핵심 메커니즘인 방 분배와 인질 교환을 구현한다. 이전 스토리들로 게임 흐름이 완성되었다면, 이 스토리로 실제 게임 규칙을 완전히 구현한다.

**Independent Test**: 게임 시작 시 플레이어들이 두 방으로 분배되고, 라운드 종료 시 방장들이 인질을 선택하여 교환하면, 다음 라운드에서 해당 플레이어들이 반대편 방에 있는지 확인한다.

**Acceptance Scenarios**:

1. **Given** 게임이 시작되었을 때, **When** 라운드 1이 시작되면, **Then** 플레이어들이 빨간 방과 파란 방으로 가능한 균등하게 분배된다.
2. **Given** 플레이어가 빨간 방에 배정되었을 때, **When** 자신의 화면을 확인하면, **Then** 자신이 속한 방 이름과 같은 방에 있는 다른 플레이어 목록이 표시된다.
3. **Given** 라운드가 종료되었을 때, **When** 방장들이 인질 교환을 위해 각자 1명씩 선택하면, **Then** 선택된 플레이어들은 다음 라운드에서 반대편 방으로 이동한다.
4. **Given** 라운드 3에서 인질 교환 시간일 때, **When** 방장이 교환할 인질 수를 확인하면, **Then** 라운드 번호에 따라 교환 가능한 인질 수가 증가한다(예: 라운드 1은 1명, 라운드 2는 2명).
...
마지막 라운드 종료 시 대통령과 폭탄범이 같은 방에 있는지 확인하여 승리 팀을 자동으로 판정하고 결과를 표시한다.

**Why this priority**: 게임의 최종 목표를 달성하고 승패를 명확히 한다. 모든 게임 메커니즘이 구현된 후 최종 완성도를 높인다.

**Independent Test**: 마지막 라운드 종료 시 대통령과 폭탄범의 위치를 확인하고, 같은 방에 있으면 레드 팀 승리, 다른 방에 있으면 블루 팀 승리를 표시하는지 검증한다.

**Acceptance Scenarios**:

1. **Given** 마지막 라운드가 종료되었을 때, **When** 대통령과 폭탄범이 같은 방에 있으면, **Then** "레드 팀 승리!" 메시지가 모든 플레이어에게 표시된다.
2. **Given** 마지막 라운드가 종료되었을 때, **When** 대통령과 폭탄범이 다른 방에 있으면, **Then** "블루 팀 승리!" 메시지가 모든 플레이어에게 표시된다.
...
- **방장 퇴장**: 방장이 게임 중 퇴장하면 다음 플레이어에게 자동으로 방장 권한이 이전된다.
...
- **인질 선택 제한 시간 초과**: 방장이 제한 시간 내에 인질을 선택하지 않으면 자동으로 무작위 선택되거나 교환 없이 다음 라운드로 진행한다.
...
- **FR-005**: 시스템은 게임 시작 시 플레이어들을 레드 팀과 블루 팀으로 가능한 균등하게 배정해야 한다.
- **FR-006**: 시스템은 정확히 1명의 대통령(블루 팀)과 1명의 폭탄범(레드 팀)을 무작위로 배정해야 한다.
...
- **FR-011**: 시스템은 라운드 종료 시 방장들이 인질을 선택할 수 있는 인터페이스를 제공해야 한다.
- **FR-012**: 시스템은 라운드 번호에 따라 교환 가능한 인질 수를 자동으로 설정해야 한다(예: 라운드 1은 1명, 라운드 2는 2명).
- **FR-013**: 시스템은 마지막 라운드 종료 시 대통령과 폭탄범의 위치를 비교하여 승리 팀을 자동 판정해야 한다.
- **FR-014**: 시스템은 게임 결과 화면에서 모든 플레이어의 역할과 팀을 공개해야 한다.
- **FR-015**: 시스템은 플레이어 연결 상태를 모니터링하고 연결 끊김 시 다른 플레이어들에게 알려야 한다.
...
- **플레이어 (Player)**: 닉네임, 연결 상태, 방장 여부
- **게임 세션 (Game Session)**: 참가 플레이어 목록, 현재 라운드 번호, 라운드 시작 시간, 팀 배정 정보, 방 배정 정보
- **역할 카드 (Role Card)**: 역할 이름(대통령, 폭탄범, 일반), 팀 색상(레드/블루), 배정된 플레이어
- **방 (Room in Game)**: 방 이름(빨간 방/파란 방), 현재 라운드의 플레이어 목록
- **인질 교환 (Hostage Exchange)**: 라운드 번호, 교환할 플레이어 목록, 각 방의 방장 선택 정보
...
- 방 코드는 24시간 동안 유효하며, 게임 종료 후 1시간 동안 재사용 가능하다.
